...
function drawHands(){
  redHandEl.innerHTML="🔴 "+hands.red.map(c=>`<span class="card ${current==='red'?'':'disabled'}" onclick="${current==='red'?`selectCard('${c}')`:''}">${c}</span>`).join("");
  blueHandEl.innerHTML="🔵 "+hands.blue.map(c=>`<span class="card ${current==='blue'?'':'disabled'}" onclick="${current==='blue'?`selectCard('${c}')`:''}">${c}</span>`).join("");
}
...
function playMove(i){
  ...
  const cell=board[i];
  if(selectedCard==="JE"){ if(cell.chip&&cell.chip!==current)cell.chip=null; else return; }
  else if(selectedCard==="JEE"){ if(!cell.chip)cell.chip=current; else return; }
  else{ if(cell.card!==selectedCard||cell.chip)return; cell.chip=current; }
  ...
  if(checkSequence(current)){
    ...
    if(scores[current]===2){alert(current+" wins!");location.reload();}
  }
...
function checkSequence(color){
  ...
  for(let i=0;i<100;i++){
    ...
    for(const[dX,dY]of dirs){
      ...
      if(cells.length===5){cells.forEach(x=>sequenceCells.add(x));return true;}
    }
  }
  return false;
}
